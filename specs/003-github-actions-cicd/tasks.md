...
**Status as of 2026-07-01**: T029, T039 complete (see notes above); dev deploys (V1–V5, V8–V10) are proven live. **Still blocked**: T041 is a manual Azure DevOps admin action that should only happen once **both** environments are proven, not just dev — `promote-prod.yml` has never been run (V6/V7 deferred since T030). T042 (full V1–V10 + rollback drill) is nearly ready but likewise needs the production promotion + rollback exercised first, since V6/V7 are its only remaining unverified scenarios.
