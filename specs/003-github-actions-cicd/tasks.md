...
- [ ] T011 [US1] Configure a branch protection rule on `main` (repo Settings → Branches) requiring the `ci` status check to pass before merge
- [ ] T012 [US1] Verify (quickstart.md V1): a PR with a failing test fails `ci` and is blocked from merging; a passing PR merges; the full run completes in under 10 minutes

**Blocked (not completed in this pass)**: T011 requires a GitHub repository to exist (this repo currently has only an Azure DevOps remote — see the sequencing note at the end of this file); T012's local-equivalent checks (lint/build:ci/test) were verified directly in T004, but the actual "PR blocks merge on GitHub" behavior can't be proven without a live repo + branch protection rule.
...
- [ ] T019 [US4] Execute the history-purge runbook (T018): run `git filter-repo`, force-push the rewritten history, and rotate the Power Platform service-principal client secret
- [ ] T020 [US4] Verify (quickstart.md V2, V8): a full working-tree + history secret scan (e.g. `gitleaks detect --source . --log-opts="--all"`) finds zero secrets/identifiers; a PR adding a fake key is blocked by the `gitleaks` step from T017

**Blocked (deferred to a live, user-driven walkthrough)**: T019 is destructive (rewrites all commit hashes, requires a coordinated force-push) and its "rotate the credential" clause requires Power Platform admin access this environment doesn't have — the project owner chose to run this interactively rather than have it executed unattended. T020 depends on T019 having run, and `gitleaks` is not installed in this local environment (it runs via GitHub Actions in CI, not locally).
...
- [ ] T022 [US2] Create GitHub Environments `dev` (no required reviewers) and `production` (required reviewers = release owners) in repo Settings → Environments, each populated with secrets `PP_CLIENT_ID`, `PP_CLIENT_SECRET`, `PP_TENANT_ID` and variables `PP_ENVIRONMENT_URL`, `PP_ENVIRONMENT_ID`, `PP_APP_ID` per data-model.md
...
**Blocked (deferred to a live, user-driven walkthrough)**: T021 (provisioning a real Power Platform service principal) and T022 (creating GitHub Environments/secrets, which also requires a GitHub repo to exist) need admin access this environment doesn't have — the project owner chose to do these interactively. T029/T030 need a real deployed environment and a live repo to verify against, so they remain open until T021/T022 (and the GitHub repo itself) exist.
